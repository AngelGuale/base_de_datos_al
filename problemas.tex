...
\problem{}
Ratifique o rectifique las siguientes \textbf{definiciones}.
\begin{enumerate}[a.]
\item Se dice que los vectores \kvect{v}{n} de un espacio vectorial $V$ son linealmente dependientes, si y sólo si, al menos uno de ellos se puede escribir como combinación lineal de los $n-1$ vectores restantes.
\item Sean $V$ y $W$ espacios vectoriales sobre un campo $K$. Se dice que $T: V\rightarrow W$ es una transformación lineal si se cumple que:
$$\forall v_1,\,v_2 \in V\qquad T(v_1 + v_2) = T(v_1) + T(v_2)$$
$$\forall v_1,\,v_2 \in V\qquad T(v_1 - v_2) = T(v_1) - T(v_2)$$

\end{enumerate}

\problem{}
Demuestre que la siguiente proposición: “si $A \in \mmxn$, entonces la imagen de $A$ es igual al espacio columna de $A$”

\problem{}
Califica como verdadera o falsa cada proposición que se enuncia a cntinuación. Justifique su respuesta.
\begin{enumerate}[a.]
\item Sean $H$ y $W$ subespacios de un espacio vectorial $V$. Si $V = H \oplus W$, entonces $V=H\cup W$
\item Sean $v_1, v_2, v_3$ vectores de un espacio vectorial $V$. Si $u_1, u_2, u_3$ son linealmente independientes y son, respectivamente, los vectores coordenados de $v_1, v_2, v_3$ respecto de una base $B$ de $V$ entonces $dim V\geq 3$
\end{enumerate}

\problem{}
Sea el espacio vectorial real $V = \mdosxdos$. Sean los subespacios de $V$:
$$H_1 = \left\{\matrdxd{a & b}{c & d} \in \mdosxdos/ c=2a+b,\, d=a-b \right\}$$
$$H_2 = gen\left\{\matrdxd{1 & -1}{5 & 1},\matrdxd{2 & 2}{2 & 1}\right\}$$
$$H_3\left\{\matrdxd{a & b}{c & d} \in \mdosxdos/ a=3c-8b-5d\right\}$$
\begin{enumerate}[a.]
\item Encuentre una base y determine la dimensión de $H_1 \cap H_2$.
\item Determine si $H_1 \cup H_3$ es un subespacio de $V$. Justifique su respuesta.
\item ¿$\matrdxd{-8 & -5}{19 & 4} \in H_2 + H_3$? Justifique su respuesta.
\end{enumerate}

\problem{}
Sea $f: \rdos \rightarrow \rdos$ una función con regla de correspondencia 
$$f\matrdxd{x}{y}=\matrdxd{2x-y}{y-x}$$
\begin{enumerate}[a.]
\item Pruebe que $f$ es un operador lineal en \rdos.
\item Suponga que a cada punto de la recta definida por la ecuación $2x+y=4$ se le aplica el operador $f$. Encuentre la ecuación del nuevo lugar geométrico y grafíquelo.
